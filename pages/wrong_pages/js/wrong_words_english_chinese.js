...
new Vue({
    ...
    methods:{
        chinese_options_consturctor(){
            ...
            this.english_question = this.vocabularyList[this.answer_index].word

            this.answer = Math.floor(Math.random()*4)
            if(this.answer == 0){
                this.A = this.vocabularyList[this.answer_index].meaning
                this.B = this.vocabularyList[ran2].meaning
                this.C = this.vocabularyList[ran3].meaning
                this.D = this.vocabularyList[ran4].meaning
            }else if(this.answer == 1){
                this.A = this.vocabularyList[ran2].meaning
                this.B = this.vocabularyList[this.answer_index].meaning
                this.C = this.vocabularyList[ran3].meaning
                this.D = this.vocabularyList[ran4].meaning
            }else if(this.answer == 2){
                this.A = this.vocabularyList[ran3].meaning
                this.B = this.vocabularyList[ran2].meaning
                this.C = this.vocabularyList[this.answer_index].meaning
                this.D = this.vocabularyList[ran4].meaning
            }else if(this.answer == 3){
                this.A = this.vocabularyList[ran4].meaning
                this.B = this.vocabularyList[ran2].meaning
                this.C = this.vocabularyList[ran3].meaning
                this.D = this.vocabularyList[this.answer_index].meaning
            }else{
                console.log('error')
            }
        },
    ...
    async created(){
        ...
        var length = Object.keys(this.wrongData.words).length
        if(length < 4){
            swal({
                title:"错题数量不大够喵",
                text:"再去练练吧,说不定有错呢"
            ...
            for(let i = 0; i < length; i++){
                this.vocabularyList.push(this.wrongData.words[Object.keys(this.wrongData.words)[i]])
            }
            this.chinese_options_consturctor()
        }
    }
